...
import { executeAzureOpenAIApi, callApiWithRetry } from '../llmUtils';

export class AzureOpenAITranslator implements TranslationProvider {
    name = 'azureopenai';
    ...
    async translate(text: string, targetLanguage: string, settings: AITranslatorSettings, progressReporter: ProgressReporter): Promise<string> {
        ...
        return callApiWithRetry(
            ...
            settings,
            progressReporter,
            (config, p, c, signal, s) => executeAzureOpenAIApi(config, p, c, signal, s, progressReporter)
        );
    }
}